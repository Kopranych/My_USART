...
{
	RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_GPIOB, ENABLE);//включаем тактирование шины AHB1 на ней порты D и A
	...
	GPIO_Init(PORT_SIG, &PORT_LCD);// инициализируем сами порты
...
//	PORT_SIG &= ~(ON<<RW);
	...
	temp |= (data &MASK_LOW)<<1;// обрезали младщую тетраду данных 
	...
	temp|= (tmp & MASK_HIGH)<<5;// обрезали старшую тетраду
